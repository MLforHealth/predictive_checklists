...
import tqdm
import shlex
from scripts import experiments, launchers
import os
...
class Job:
    ...
    def __init__(self, train_args, sweep_output_dir, slurm_pre, script_name, experiment_name):
        args_str = json.dumps(train_args, sort_keys=True)
        args_hash = hashlib.md5(args_str.encode('utf-8')).hexdigest()
        self.output_dir = os.path.join(sweep_output_dir, train_args['dataset'], experiment_name, args_hash)

        self.train_args = copy.deepcopy(train_args)
        self.train_args['output_dir'] = self.output_dir
        self.train_args['experiment_name'] = experiment_name

        command = ['python -m', 'scripts.' + script_name[:script_name.rfind('.')]]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_slurm_jobs', type=int, default = 200)
    ...
    if args.command == 'launch':
        ...
        to_delete = [j for j in jobs if j.state in [Job.INCOMPLETE, Job.DONE]]
        print(f'About to delete {len(to_delete)} jobs.')
        if not args.skip_confirmation:
            ask_for_confirmation()
        Job.delete(to_delete)